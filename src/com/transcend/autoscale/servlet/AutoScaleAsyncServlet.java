 ...
 * All Rights Reserved.
 */
package com.transcend.autoscale.servlet;
...
/**
 * Servlet to receive AutoScale requests.
 */
@WebServlet(
// servlet name
name = "autoscale-async", displayName = "AutoScale request servlet",
loadOnStartup = 1,
// servlet url pattern
urlPatterns = { "/AsyncAction/" },
// async support needed
asyncSupported = true)
public class AutoScaleAsyncServlet extends BaseAsyncServlet
    implements ServiceResponseListener {
    private final Logger logger = Appctx
            .getLogger(AutoScaleAsyncServlet.class.getName());
     ...
     */
    public AutoScaleAsyncServlet() {
        super();
    }

    @Override
    public void init(ServletConfig config) throws ServletException {
        logger.info("Initializing autoscale servlet.");
        super.init(config);
    }
    ...
    protected void enqueLongRunningTask(final ServiceRequest request,
        ...
        try {
            final AsyncServiceImpl impl = Appctx.getBean("autoscaleServiceAsync");
            register(impl);
